...
**A parameter that doesn't apply to the item's kind now fails the call** rather than being silently dropped (root CLAUDE.md's Cross-kind fields at the boundary, Stage 7 of `docs/typed-item-params-plan.md`). `points` or `priority` on an `EVENT`, `eventType` on a `TASK`, a date on a `SIMPLE`: each comes back as an error naming the parameter and the kind, which is a strictly better outcome for a model client than a success response that quietly kept half the request. Nothing in this server needs a change for that — it already sends only what the caller supplied — but tool descriptions that describe a parameter as merely "only meaningful on ..." are now understating it.
