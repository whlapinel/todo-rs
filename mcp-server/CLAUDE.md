...
`create_item`/`update_item`/`create_item_series`/`update_item_series`'s `priority` parameter (root CLAUDE.md's Priority section) is a plain passthrough number, 1–4 — unlike `points`/`assignedToUserId`, it needs no team-backed-project caveat in its description, since the server enforces no such restriction on it.

**A parameter that doesn't apply to the item's kind now fails the call** rather than being silently dropped (root CLAUDE.md's Cross-kind fields at the boundary, Stage 7 of `docs/archived/typed-item-params-plan.md`). `points` or `priority` on an `EVENT`, `eventType` on a `TASK`, a date on a `SIMPLE`: each comes back as an error naming the parameter and the kind, which is a strictly better outcome for a model client than a success response that quietly kept half the request. Nothing in this server needs a change for that — it already sends only what the caller supplied — but tool descriptions that describe a parameter as merely "only meaningful on ..." are now understating it.

One parameter is exempt and must stay so: `complete` is in `update_item`'s `required` list, so a model renaming an Event has no way not to send it. The server accepts `complete: false` on any kind and rejects only `complete: true` on a non-Task. Do not make `complete` optional here without checking that exemption still holds on the server side.

**Series sub-item tools**: `list_item_series_children`/`create_item_series_child`/`update_item_series_child`/`delete_item_series_child` cover the `item_series_children` definition rows a Task series owns — lead-time preparation work fanned out onto every occurrence ("book venue, 30 days before"). `daysBefore` is non-negative and passed through as-is, unlike `create_item`'s `daysBeforeDue`: the API's own `daysBefore` field is already the non-negative "days before" number, so there is no sign flip to do here — the server negates it into `dueOffsetDays` only at materialization. There is no `get_item_series_child` tool, matching the API: a series' definition set is small and always read whole.
