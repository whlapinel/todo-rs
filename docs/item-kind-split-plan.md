...
**Status: complete.** Stages 0–7 landed (Stage 7 evaluated and not adopted — see its write-up below); Stage 8 (cleanup sweep + CLAUDE.md rewrite) closes the plan out — see its write-up below. This is the canonical copy — edit this file in place as stages complete. It originated in a Claude Code plan-mode session (`~/.claude/plans/splendid-wishing-sphinx.md`), copied here so it survives independently of that session.
...
- [x] Stage 8 — Remove now-provably-dead `Option`-returning delegation methods; update CLAUDE.md's Domain Models section (it's already stale re: the pre-`ItemType` shape, and will need a rewrite reflecting this plan's final shape)
...
**Stage 6 as implemented — no corrections to the plan.** Landed exactly as scoped:

- `src/domain/item.rs`: `google_event_id`/`calendar_subscription_id: Option<String>` added to `EventItem` (removed from `Item`'s own flat fields); `ItemType::google_event_id()`/`calendar_subscription_id() -> Option<&str>` added (matches `Event` only, `None` for every other kind); `Item::google_event_id()`/`calendar_subscription_id() -> Option<String>` delegation methods added following `series_id()`'s exact owned-`String` convention. No `Item::set_google_event_id()` — every write site matches into `ItemType::Event` directly, same as `TaskItem::complete`/`series_id` before it.
- The four guard checks flagged by Stage 0's audit (`current.google_event_id.is_some()` in `update_item`/`delete_item`, `src/service/items.rs`, and their `team_items.rs` twins) were updated to `current.google_event_id().is_some()` — the plan's own sketch suggested a `matches!(&current.item_type, ItemType::Event(e) if e.google_event_id.is_some())` instead, but the simpler delegation-method form is exactly equivalent (the method already returns `None` for every non-`Event` kind) and matches the convention every prior stage actually used at its own guard/read sites, so the more verbose form wasn't needed.
- `build_item_type` (`service::items.rs`) and its `service::team_items.rs` twin both hardcode `google_event_id: None, calendar_subscription_id: None` in their `Event` arm, rather than threading a new parameter through the way Stage 3 threaded `complete` — unlike `complete`/`series_id`, these two fields are never settable via `CreateItemParams`/`UpdateItemParams`/`CreateTeamItemParams`/`UpdateTeamItemParams` at all (no Smithy field, no CLI flag, no MCP parameter — only `service::calendar_sync` ever writes them, directly onto an `Item` it constructs itself, never through `build_item_type`). Safe to hardcode `None` on the update path too: `update_item`/`update_team_item`'s `current.google_event_id().is_some()` guard (previous bullet) means `build_item_type` is never even reached for an already-imported item's update.
- `row_to_item` (`src/storage/sqlite/mod.rs`) reads both fields once from the row and threads them into the `Event` match arm only (previously read directly into the flat `Item` literal).
- `src/storage/sqlite/items.rs`'s six `.bind(&item.google_event_id)`/`.bind(&item.calendar_subscription_id)` call sites (three INSERT/UPDATE statements × two fields) became `.bind(item.google_event_id())`/`.bind(item.calendar_subscription_id())` — dropped the `&`, matching `item.series_id()`'s no-reference precedent from Stage 5.
- `service::calendar_sync.rs`: `build_imported_item` and the update-on-sync-drift branch in `run_diff` both moved the two fields into their `EventItem { .. }` literal (the update branch round-trips `existing_item.google_event_id()`/`calendar_subscription_id()`, the same way it already round-tripped `series_id()`); the `existing_by_uid` lookup map's key type changed from `HashMap<&str, &Item>` to `HashMap<String, &Item>` since `Item::google_event_id()` now returns an owned `Option<String>` rather than borrowing from the (now nonexistent) flat field; the `run_diff` deletion-scan loop's `let Some(uid) = item.google_event_id.as_deref()` became `let Some(uid) = item.google_event_id()` (owned) with `seen_uids.contains(uid.as_str())` at the call site below it.
- `src/storage/sqlite/items.rs` test module gained one new helper, `set_google_event_id(item, uid, sub_id)`, following `set_parent_item_id`'s exact precedent — turns a plain (Task-defaulted) test fixture into an `Event` with both calendar-import fields set, since a bare `item.google_event_id = ..` assignment is no longer expressible once the field only exists on `EventItem`.
- ~25 call sites total across `src/service/{items,team_items,calendar_sync}.rs`, `src/storage/sqlite/{mod,items}.rs`, `src/json_api/project_items.rs`, and `src/web_ui/{components/row,project_events/{handlers,templates},project_tasks/templates}.rs` — all mechanical (`.google_event_id`/`.calendar_subscription_id` field read → method call; direct `Item` field assignment in test fixtures → nested `EventItem { .. }` literal; two doc-comment references to the old `Item::google_event_id.is_some()` shape corrected to `google_event_id().is_some()` for accuracy). Zero edits to `templates/*.html` (confirmed via `git diff --stat -- templates/`), matching every prior stage's precedent. `cargo fmt` (plain, repo-root), `cargo test` (601 passed, 0 failed — same count as Stages 3–5, since this stage's changes are covered by fixing the existing four calendar-import guard tests and the three storage-layer round-trip regression tests in place rather than adding new ones), and `task check` all clean — only the same pre-existing dead-code warnings noted in Stages 1/3/4's write-ups remain, none new.
...
**Stage 8 as implemented — first bullet is a no-op, given Stage 7's own outcome.** The cleanup sweep was scoped to methods made redundant by Stage 7's direct-struct-access convention specifically — but Stage 7 was evaluated and not adopted (no screen ever switched `require_task`/`require_event`/`require_simple`/`require_project_template` to hand back a concrete payload struct), so there is no such convention anywhere in the codebase for anything to be redundant against. Confirmed via `cargo check --all-targets`: the only dead-code warnings present are `from_kind`/`recurrence_mut` (`ItemType`) and `new_task`/`new_event` (`Item`), plus the handful of unrelated pre-existing warnings already noted in Stages 1/3/4/5's write-ups (`item_series.rs`'s stray import, `storage/sqlite/users.rs`'s unused `Row`, `ProjectMember`/`TeamMember` never constructed, etc.). None of the four newly-checked names are "`Option`-returning `Item` delegation methods" in the sense this bullet meant — `from_kind`/`new_task`/`new_event` are constructors, not readers, and `recurrence_mut` is a mutable-accessor on `ItemType` (not `Item`) that nothing in this plan's stages ever routed through. They predate this stage (present since Stage 1, when `ItemType`'s per-kind structs were first introduced) and aren't a consequence of Stage 7's convention, so removing them is out of this bullet's actual scope — left in place rather than opportunistically deleted, matching the plan's own "don't remove any that are still genuinely used by cross-kind generic code" caution applied to the analogous case of "not actually caused by what this bullet is about." No code changed for this bullet.

Second bullet: CLAUDE.md's "Domain Models" section (root `CLAUDE.md`) was rewritten in place to describe the actual final shape — the `Item` envelope + `ItemKind`/`ItemType` split, the four payload structs (`TaskItem`/`EventItem`/`TemplateItem`/`SimpleItem`) and the three structs shared across them (`Schedule`/`Recurrence`/`TeamAssignment`), the `HasSchedule`/`Completable` traits and why they're `#[allow(dead_code)]`, and `Item`'s own delegation-method API — including a callback to Stage 7's finding (that the delegation methods deliberately stay `Option`-returning even for a statically-known kind, since e.g. `parent_item_id` is genuinely optional on a `Task`, not just optional across kinds). The section's existing `Item.series_id` paragraph was updated to describe its Stage 5 move onto `TaskItem`/`EventItem` rather than the flat-field shape it used to have. Read back against `src/domain/item.rs` line by line before committing, per this bullet's own verification note. No other CLAUDE.md section was touched — the neighboring "Recurrence" section already described `Recurrence` as "embedded in `ItemType::Task`/`Event`/`Template`" (it was evidently kept in sync incrementally as earlier stages landed), so it needed no correction.

`cargo fmt` (plain, repo-root), `cargo build`, `cargo test`, and `task check` all clean — same warning set as the sweep-bullet paragraph above, none new. This closes out `docs/item-kind-split-plan.md`; all 9 stages (0–8) are done.
