...
- Add a New-item button to the cross-project Tasks/Events screens (`src/web_ui/all_projects_tasks.rs` at `/web/tasks`, `src/web_ui/all_projects_events.rs` at `/web/events` — added by `docs/all-projects-landing-plan.md`, which superseded the cross-project Home calendar these used to target), mirroring the per-project screens' `+ Task`/`+ Event` header buttons. `docs/all-projects-landing-plan.md`'s Context section deliberately excluded this from its own scope ("creation is inherently project-scoped; a project-picker-driven creation flow is separately tracked... not built here") — same reasoning `docs/calendar-day-drawer-plan.md` gave for excluding it from the old Home calendar before that. Needs a design pass neither plan did: since these screens aren't scoped to one project, the New button needs some way to pick which project the new item goes into (e.g. a project-picker dropdown before landing on `/tasks/new`/`/events/new`, or defaulting to the user's most-recently-active project) — not just a straight copy of the per-project version's two-link segmented control. This is now the same gap as the item above (new-item dialog with a project selector) — likely one piece of work, not two.
