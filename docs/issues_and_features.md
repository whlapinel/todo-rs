...
- Bug found while implementing Stage 3 of `docs/calendar-day-drawer-plan.md` (2026-08-22): `render_rows`'s assigned-to-me filter in `src/web_ui/project_dashboard.rs` (the per-project Dashboard's flat list view) is `assigned_to_any || item.assigned_to_user_id() == Some(user_id)`, with no gate for personal (non-team-backed) projects. A personal-project item never carries a `TeamAssignment` at all (confirmed in `src/domain/item.rs`/`src/storage/sqlite/mod.rs`'s row mapping), so `assigned_to_user_id()` is always `None` for one — meaning this filter silently excludes *every* item on a personal project's Dashboard list by default (`assignedToAny` unchecked), unless the user manually checks "Assigned to Any" every time. The Stage 3 calendar work added the equivalent filter gated on `is_team_project` (`day_list_rows`/`build_calendar_days` in the same file) specifically to avoid repeating this bug, but `render_rows` itself was left untouched (outside that stage's scope). Fix: gate `render_rows`' filter the same way — `!is_team_project || assigned_to_any || ...`.
