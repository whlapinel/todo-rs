...
- Reschedule/Assign (via `components/reschedule_dialog.html`/`quick_assign_dialog.html`) save through the item's own generic resource route, whose response re-renders via the plain `ProjectTaskRow`/`ProjectEventRow` shape rather than the calendar's `type_badge`/`parent_name`/`project_name` overlay (see `project_calendar.rs`/`main_calendar.rs`'s `calendar_row`) — so a row saved via Reschedule/Assign from a calendar day-drawer briefly loses that styling until the drawer's next reload. Edit/Duplicate are unaffected since both already navigate away to the Tasks/Events list. Fixing properly would mean threading a `view=`-style render-context param (mirroring the one `complete_project_item_series_occurrence_form` already uses for virtual occurrences) through the generic item-update handlers so they know to re-render the calendar-flavored row when called from a calendar context.
