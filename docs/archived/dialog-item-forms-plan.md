...
Status: **done** — Stages 0–3 all landed 2026-08-23. Written 2026-08-23 from the first entry in `docs/issues_and_features.md`, which bundles several tenuously-related asks and explicitly invites splitting into stages — this doc does that split and records the design decisions made along the way.
...
**Status: done** (2026-08-23). `cargo build`/`cargo fmt`/`cargo test` clean (465 passed, same
count as Stages 0–2 — this stage is new entry points over already-tested `create_project_task_form`/
`create_project_event_form` creation logic, no new service-layer tests needed); `task web-styles`
run, no `static/style.css` diff (every class used, including the project-`<select>` markup, was
already present from the per-project new-item dialogs). Not yet verified live in a browser — the
user's own pass per CLAUDE.md's standing policy.

Closes both this doc's opening line and the later "Add a New-item button to the cross-project Tasks/Events screens" entry.

Landed essentially as planned below, with these notes for anyone resuming:

- **Project-resolution cascade is a shared helper**, not duplicated per screen: `crate::web_ui::resolve_new_item_project` (`src/web_ui/mod.rs`) takes `&[Project]` + the query `project` param + `personal_project_id` and returns the first match in that priority order, else the list's first project. Both `all_projects_tasks::new_all_projects_task_dialog` and `all_projects_events::new_all_projects_event_dialog` call it — this one cascade is identical in both screens (unlike the genuinely screen-specific row/filter helpers elsewhere in this codebase that follow the "duplicate small per-screen helpers" precedent), so it was judged worth sharing rather than copying. Returns `None` only if the requester belongs to zero projects, mapped to `ItemError::NotFound` — not expected in practice since login-time `ensure_default_project` (Stage 0) guarantees at least one, but there's no other sane fallback if it somehow happened.
- **New handlers**: `GET /web/tasks/new` → `all_projects_tasks::new_all_projects_task_dialog`, `GET /web/events/new` → `all_projects_events::new_all_projects_event_dialog`. Both fetch `user_projects` via `project_service::list_projects`, fetch the requester's own `User` via `UserRepo::get` (for `personal_project_id`), resolve the selected project via the shared helper above, and render a new `AllProjectsNewTaskPageTemplate`/`AllProjectsNewEventPageTemplate` (defined inline in each handler's own `.rs` file, matching those files' existing convention of no separate `templates.rs` module). The Task variant recomputes `assignee_options`/`is_team_admin` for whichever project is currently selected, exactly like `project_tasks::new_project_task_page` already does for a single project. The Event variant has no team-only fields at all (Events' new-item form never had assignee/points fields to begin with — see `project_events/new_page.html`), so nothing besides `project_id`/`project_options` varies with the selection.
- **Dynamic re-render, not a dynamic POST url**: landed as the plan's own "simplest correct approach" — the `<select name="project">` has `hx-get` back to the *same* `/web/tasks/new` (or `/web/events/new`) route, `hx-trigger="change"`, `hx-include="this"` (scopes the request to just the select's own value, regardless of its being nested inside the `<form>`), `hx-target="#action-dialog"`, `hx-select="#dialog-fragment"` — the whole dialog fragment is re-rendered server-side for the newly selected project, including a fresh `hx-post="/web/projects/{{ project_id }}/tasks"` on the form itself. No client-side URL-rewriting JS was needed, since the form's `hx-post` is just a template value baked in on every render.
- **Trigger buttons**: `+ New Task`/`+ New Event` icon buttons added to `templates/all_projects_tasks/list_page.html` / `all_projects_events/list_page.html`'s header row (`all_projects_events/list_page.html` had no header-right section at all before this — added one), `hx-get`-ing the `new` route straight into `#action-dialog` with `hx-select="#dialog-fragment"`, same shape as every other Stage 1/2 trigger. The Tasks button preserves `?showComplete=1` the same way `project_tasks/list_page.html`'s own `+ Task` button does.
- **`detail_via_dialog` opt-in closed the loop**: `all_projects_tasks::all_projects_task_row` had been forcing `row.detail_via_dialog = false` since Stage 1 (with a comment deferring it to this stage) even though `ProjectTaskRow::from_item` already sets it `true` — that override is now deleted, so all-projects Task rows open the read-only detail dialog like every other opted-in screen. `all_projects_events::all_projects_event_row` needed no equivalent change: it's built on `ProjectEventRow::from_item`, which Stage 2 already hardcoded to `detail_via_dialog: true` unconditionally (no per-call-site override existed there to remove).
- No new Smithy/service-layer changes — `create_project_task_form`/`create_project_event_form` (the same per-project handlers `project_tasks`/`project_events` already use) are the actual creation targets; this stage is purely new GET entry points + templates + trigger buttons.
