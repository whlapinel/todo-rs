...
- `POST /web/scheduled-overdue/reschedule` — re-fetches the same eligible set server-side (never trusts a client-submitted id list, so the batch always matches what's currently displayed), and for each item:
  ...
  2. Builds an `EditItem` by round-tripping every other field from the current item, overriding only `scheduled_date`/`scheduled_end_date`. (Written against `UpdateProjectItemParams` and `project_dashboard.rs`, both since deleted — the flat params by Stage 8 of `docs/archived/typed-item-params-plan.md`, the screen by the Project abstraction. The round-trip itself is now `EditTask::from_item` plus the one overlaid field; see the toggle handlers in `src/web_ui/project_calendar.rs` for the shape.)
