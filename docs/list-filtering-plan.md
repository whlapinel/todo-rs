...
Status: **Stage 1 done and committed; Stage 2 not started.** Scopes the "Need filtering for all lists" entry in `docs/issues_and_features.md`, narrowed to a single screen first — see Out of scope.
...
- `matches`'s actual signature ended up as `matches(&self, item: &Item, requester_user_id: &str, is_team_project: bool, now: DateTime<Utc>) -> bool` — two params beyond the original sketch (`is_team_project` to gate the assignment check, `now` for the due/schedule comparisons, passed in rather than read via `Utc::now()` so the function stays unit-testable). Any Stage 2 call site needs to supply both.
- **Nothing outside `list_filters.rs` has been touched yet.** `project_tasks_page`, `list_task_rows_for_project`, `render_rows_with_virtual`, `ProjectTaskRow::from_item`, `ProjectTaskForm`, `ShowCompleteQuery`, and every template under `templates/project_tasks/` are all still exactly as they were — `showComplete` is still the only working filter end-to-end. Stage 2 below is the entire remaining scope of this plan.
- Verified: `cargo fmt` (repo-root, no path args, per this repo's formatting policy) made no changes; the 14 new tests pass; only pre-existing, unrelated warnings remain elsewhere in the tree (dead-code lints on the new module itself are expected until Stage 2 wires it in — ignore those until then).
...
### Stage 2 — wire into Project Tasks: page load, list rebuild, filter bar (not started — pick up here)
...
| `src/web_ui/project_tasks/handlers.rs` | `project_tasks_page`, `update_project_task_form`, `complete_project_item_series_occurrence_form`, dialog handlers — swap `show_complete`/`ShowCompleteQuery` for `ListFilters` | not started |
| `src/web_ui/project_tasks/mod.rs` | `list_task_rows_for_project`, `render_rows_with_virtual`, `ProjectTaskForm`, `create_params_from_form`/`update_params_from_form`'s `show_complete` passthrough | not started |
| `src/web_ui/project_tasks/templates.rs` | `ProjectTaskRow::from_item`, `Row`'s URL builders (`list_query` and friends) | not started |
| `templates/project_tasks/list_page.html` | Filter bar form + controls | not started |
| `templates/project_tasks/new_page.html` | Round-trip full filter set via hidden fields (was just `showComplete`) | not started |
| `templates/macros.html` | Shared select styling reused by the new filter controls, if not already generic enough | not started |
...
- Remaining (Stage 2, once wired in): `cargo test` for the full suite (regression check — nothing in Stage 1 should need new tests, but Stage 2's changes to `project_tasks` will). Manual smoke test via `task run`: on a team-backed project with a mix of assigned/unassigned/overdue/scheduled-past/recurring tasks, confirm each filter control narrows the list correctly alone and in combination with `showComplete`; confirm a filtered URL is bookmarkable/shareable (reload with the same query string reproduces the same list); confirm completing/rescheduling a row from a filtered view keeps the same filters applied on the re-rendered row/list rather than resetting to defaults.
