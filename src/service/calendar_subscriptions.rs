...
pub async fn create_calendar_subscription(
    ...
    let subscription = calendar_repo
        ...
        .await?;
    if let Err(e) =
        calendar_sync::sync_subscription(&subscription, item_repo.as_ref(), calendar_repo.as_ref())
            .await
    {
        tracing::warn!(
...
mod tests {
    ...
    use crate::storage::sqlite::{MockCalendarSubscriptionRepo, MockItemRepo, MockProjectRepo, MockTeamRepo};
    use chrono::Utc;
    ...
    async fn list_calendar_subscriptions_rejects_non_member() {
        ...
        let err = list_calendar_subscriptions(&projects, &teams, &calendar_repo, "p1", "someone_else")
            .await
            .unwrap_err();
        assert!(matches!(err, ItemError::Invalid(_)));
    }
    ...
    async fn delete_calendar_subscription_cascades_imported_items() {
        ...
        item_repo.expect_list_by_calendar_subscription().returning(|_| {
            Ok(vec![
                Item {
                    id: "i1".to_string(),
        ...
        delete_calendar_subscription(&projects, &teams, &calendar_repo, &item_repo, "p1", "owner1", "sub1")
            .await
            .unwrap();
    }
}
